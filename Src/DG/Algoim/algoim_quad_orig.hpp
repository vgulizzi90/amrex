...
namespace Algoim
{
    ...
    struct ImplicitIntegral
    {
        ...
        ImplicitIntegral(const Phi& phi_, F& f_, const TinyVector<bool,N>& free_, const std::array<PsiCode<N>,1 << (N-1)>& psi_, int psiCount_, const BoundingBox<Real,N>& xrange_, int p_, int level = 0)
            : phi(phi_), f(f_), free(free_), psi(psi_), psiCount(psiCount_), xrange(xrange_), p(p_)
        {
