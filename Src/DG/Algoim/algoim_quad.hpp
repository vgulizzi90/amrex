...
namespace Algoim
{
    namespace detail
    {
        ...
        struct OneDimRootFind
        {
            ...
            OneDimRootFind(const F& f_, const TinyVector<Real,N>& x_, int dim_, Real xmin, Real xmax, std::vector<Real>& roots)
                : f(f_), x(x_), dim(dim_)
            {
                Real t;
    ...
    struct PsiCode
    {
        std::bitset<N + 2> bits;
        PsiCode() {}
        PsiCode(const int& sides_val, int sign)
        {
            for (int dim = 0; dim < N; ++dim)
                bits[dim] = (sides_val == 1);
            if (sign == 0)
                bits[N] = 1;
            else
    ...
    template<int M, int N, typename Phi, typename F, bool S, typename QX, typename QW>
    struct ImplicitIntegral
    {
        ...
        void tensorProductIntegral()
        {
            for (MultiLoop<M> i(0,p); i; ++i)
            {
                ...
                for (int dim = 0, k = 0; dim < N; ++dim)
                {
                    if (free[dim])
                    {
                        x[dim] = xrange.min(dim) + xrange.extent(dim) * quadx(p, i(k));
                        w *= xrange.extent(dim) * quadw(p, i(k));
                        ++k;
                    }
                }
                f.evalIntegrand(x, w);
        ...
        void evalIntegrand(Real val, Real w) const
        {
            TinyVector<Real,N> x;
            for (int dim = 0; dim < N; ++dim) x[dim] = val;
            evalIntegrand(x, w);
        }
        void evalIntegrand(TinyVector<Real,N> x, Real w) const
        {
            ...
            for (int i = 0; i < static_cast<int>(roots.size()) - 1; ++i)
            {
                ...
                for (int j = 0; j < p; ++j)
                {
                    x[e0] = roots[i] + (roots[i+1] - roots[i]) * quadx(p, j);
                    Real gw = (roots[i+1] - roots[i]) * quadw(p, j);
        ...
        ImplicitIntegral(const Phi& phi_, F& f_, const TinyVector<bool,N>& free_, const std::array<PsiCode<N>,1 << (N-1)>& psi_, int psiCount_, const BoundingBox<Real,N>& xrange_, int p_, QX&& QuadX, QW&& QuadW, int level = 0)
            : phi(phi_), f(f_), free(free_), psi(psi_), psiCount(psiCount_), xrange(xrange_), p(p_), quadx(QuadX), quadw(QuadW)
        {
            ...
            for (int i = 0; i < psiCount; ++i)
            {
                ...
                for (int dim = 0; dim < N; ++dim)
                    if (!free[dim])
                        xint[dim].alpha = xrange(psi[i].side(dim))[dim];
                TinyVector<Interval<N>,N> gg = phi.grad(xint);

                // Determine if derivative in e0 direction is bounded away from zero.
                bool directionOkay = e0 != -1 && gg[e0].uniformSign();
                if (!directionOkay)
                {
                    if (level < 16)
                    {
                        ...
                        ImplicitIntegral<M,N,Phi,F,S,QX,QW>(phi, f, free, psi, psiCount, BoundingBox<Real,N>(xrange.min(), setComponent(xrange.max(), ind, xmid)), p, std::move(QuadX), std::move(QuadW), level + 1);
                        ImplicitIntegral<M,N,Phi,F,S,QX,QW>(phi, f, free, psi, psiCount, BoundingBox<Real,N>(setComponent(xrange.min(), ind, xmid), xrange.max()), p, std::move(QuadX), std::move(QuadW), level + 1);
                        return;
                    }
                    else
                    {
                ...
                detail::determineSigns<S>(gg[e0].alpha > 0.0, psi[i].sign(), bottomSign, topSign);
            ...
            assert(e0 != -1);
            ImplicitIntegral<M-1,N,Phi,ImplicitIntegral<M,N,Phi,F,S,QX,QW>,false,QX,QW>(phi, *this, setComponent(free, e0, false), newPsi, newPsiCount, xrange, p, std::move(QuadX), std::move(QuadW));
        }
    };

    // Partial specialisation on M=0 as a dummy base case for the compiler
    template<int N, typename Phi, typename F, bool S, typename QX, typename QW>
    struct ImplicitIntegral<0,N,Phi,F,S,QX,QW>
    {
        ImplicitIntegral(const Phi&, F&, const TinyVector<bool,N>&, const std::array<PsiCode<N>,1 << (N-1)>&, int, const BoundingBox<Real,N>&, int, QX&&, QW&&) {}
    };
    ...
    struct QuadratureRule
    {
        struct Node
        {
            TinyVector<Real,N> x;
            Real w;
            Node(const TinyVector<Real,N>& x_, Real w_) : x(x_), w(w_) {}
        };
    ...
    /* Main engine for generating high-order accurate quadrature schemes for implicitly defined domains in
       ...
       - qo specifies the degree of the underlying one-dimensional Gaussian quadrature scheme and must satisfy
         1 <= qo && qo <= 10.
    */
    template<int N, typename F, typename QX, typename QW>
    QuadratureRule<N> quadGen(const F& phi, const BoundingBox<Real,N>& xrange, int dim, int side, int qo, QX&& QuadX, QW&& QuadW)
    {
        QuadratureRule<N> q;
        std::array<PsiCode<N>,1 << (N - 1)> psi;
        
        TinyVector<bool,N> free;
        for (int n = 0; n < N; ++n) free[n] = true;
        ...
        if (0 <= dim && dim < N)
        {
            ...
            free[dim] = false;
            ImplicitIntegral<N-1,N,F,QuadratureRule<N>,false,QX,QW>(phi, q, free, psi, 1, xrange, qo, std::move(QuadX), std::move(QuadW));
        ...
        else if (dim == N)
        {
            // Surface integral
            psi[0] = PsiCode<N>(0, -1);
            ImplicitIntegral<N,N,F,QuadratureRule<N>,true,QX,QW>(phi, q, free, psi, 1, xrange, qo, std::move(QuadX), std::move(QuadW));
        }
        else
        {
            // Volume integral in the full N dimensions
            psi[0] = PsiCode<N>(0, -1);
            ImplicitIntegral<N,N,F,QuadratureRule<N>,false,QX,QW>(phi, q, free, psi, 1, xrange, qo, std::move(QuadX), std::move(QuadW));
        }
        return q;
    }
    ...
    {
        return quadGen(phi, xrange, dim, side, qo, GaussQuad::x, GaussQuad::w);
    }
} // namespace Algoim
