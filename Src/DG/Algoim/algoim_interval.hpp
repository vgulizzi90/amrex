...
namespace Algoim
{
    ...
    */
    ...
    struct Interval
    {
        ...
        Interval() : alpha(0.0), eps(0.0)
        {
            for (int dim = 0; dim < N; ++dim) beta[dim] = 0.0;
        }

        // Constructor for a constant
        explicit Interval(Real alpha_) : alpha(alpha_), eps(0.0)
        {
            for (int dim = 0; dim < N; ++dim) beta[dim] = 0.0;
        }

        // Constructor for the linear function f(y) = alpha + beta.y
        explicit Interval(Real alpha_, const TinyVector<Real,N>& beta_) : alpha(alpha_), eps(0.0)
        {
            for (int dim = 0; dim < N; ++dim) beta[dim] = beta_[dim];
        }

        // Constructor with all parameters given
        explicit Interval(Real alpha_, const TinyVector<Real,N>& beta_, Real eps_) : alpha(alpha_), beta(beta_), eps(eps_)
        {
            for (int dim = 0; dim < N; ++dim) beta[dim] = beta_[dim];
        }

        // Assignment from a constant
        Interval& operator=(Real alpha_)
        {
            this->alpha = alpha_;
            for (int dim = 0; dim < N; ++dim) beta[dim] = 0.0;
            eps = 0.0;
        ...
        Interval& operator=(const Interval& other)
        {
            ...
            alpha = other.alpha;
            for (int dim = 0; dim < N; ++dim) beta[dim] = other.beta[dim];
            eps = other.eps;
            return *this;
        }
        ...
        {
            alpha += rhs.alpha;
            for (int i = 0; i < N; ++i) beta[i] += rhs.beta[i];
            eps += rhs.eps;
            return *this;
        }
    ...
    {
        o << "Interval<" << N << ">(" << alpha.alpha << ", ";
        o << "(" << alpha.beta[0];
        for (int i = 1; i < N; ++i) o << "," << alpha.beta[i];
        o << ")";
        o << ", " << alpha.eps << ")";
        return o;
    }
