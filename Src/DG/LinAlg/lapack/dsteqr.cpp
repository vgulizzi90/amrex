...
L10:
	...
	i__1 = nm1;
	for (m = l1; m <= i__1; ++m) {
	    tst = (d__1 = e[m], std::abs(d__1));
	    if (tst == 0.) {
		goto L30;
	    }
	    if (tst <= std::sqrt((d__1 = d__[m], std::abs(d__1))) * std::sqrt((d__2 = d__[m 
		    + 1], std::abs(d__2))) * eps) {
		e[m] = 0.;
		goto L30;
	    }
...
/*     Choose between QL and QR iteration */
    ...
    if ((d__1 = d__[lend], std::abs(d__1)) < (d__2 = d__[l], std::abs(d__2))) {
	lend = lsv;
	l = lendsv;
    }
...
L40:
	if (l != lend) {
	    ...
	    for (m = l; m <= i__1; ++m) {
/* Computing 2nd power */
		d__2 = (d__1 = e[m], std::abs(d__1));
		tst = d__2 * d__2;
		if (tst <= eps2 * (d__1 = d__[m], std::abs(d__1)) * (d__2 = d__[m 
			+ 1], std::abs(d__2)) + safmin) {
		    goto L60;
		}
/* L50: */
	    }
