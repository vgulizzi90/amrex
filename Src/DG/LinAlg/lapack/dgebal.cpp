...
L140:
	...
	for (j = k; j <= i__2; ++j) {
	    ...
	    c__ += (d__1 = a[j + i__ * a_dim1], std::abs(d__1));
	    r__ += (d__1 = a[i__ + j * a_dim1], std::abs(d__1));
L150:
	    ;
	}
	ica = idamax_(&l, &a[i__ * a_dim1 + 1], &c__1);
	ca = (d__1 = a[ica + i__ * a_dim1], std::abs(d__1));
	i__2 = *n - k + 1;
	ira = idamax_(&i__2, &a[i__ + k * a_dim1], lda);
	ra = (d__1 = a[i__ + (ira + k - 1) * a_dim1], std::abs(d__1));
...
/* Computing MIN */
	d__2 = std::max(r__,g);
	if (c__ >= g || std::max(d__1,ca) >= sfmax2 || std::max(d__2,ra) <= sfmin2) {
	    goto L170;
	}
	f *= 2.;
	c__ *= 2.;
...
L180:
/* Computing MIN */
	d__1 = std::max(f,c__), d__1 = std::max(d__1,g);
	if (g < r__ || std::max(r__,ra) >= sfmax2 || std::max(d__1,ca) <= sfmin2) {
	    goto L190;
	}
	f /= 2.;
	c__ /= 2.;
