#ifndef BL_LAPACK_DLASCL_H_
#define BL_LAPACK_DLASCL_H_
...
/*     Test the input arguments */
	...
	*info = -7;
    } else if (itype <= 3 && *lda < std::max(1,*m)) {
	*info = -9;
    } else if (itype >= 4) {
/* Computing MAX */
	i__1 = *m - 1;
	if (*kl < 0 || *kl > std::max(i__1,0)) {
	    *info = -2;
	} else /* if(complicated condition) */ {
/* Computing MAX */
	    i__1 = *n - 1;
	    if (*ku < 0 || *ku > std::max(i__1,0) || ((itype == 4 || itype == 5) && *kl != *ku)) {
		*info = -3;
...
/* Computing MAX */
	    i__2 = k1 - j;
	    i__3 = k3;
	    for (i__ = std::max(i__2,1); i__ <= i__3; ++i__) {
		a[i__ + j * a_dim1] *= mul;
/* L120: */
	    }
/* L130: */
