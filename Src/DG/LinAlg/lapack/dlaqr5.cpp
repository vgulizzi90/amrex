...
/*                    .    Otherwise, use the new one. ==== */
			...
			if ((d__1 = h__[k + 2 + k * h_dim1] - refsum * vt[1], 
				std::abs(d__1)) + (d__2 = refsum * vt[2], std::abs(d__2)
				) > ulp * ((d__3 = h__[k + k * h_dim1], std::abs(
				d__3)) + (d__4 = h__[k + 1 + (k + 1) * h_dim1]
				, std::abs(d__4)) + (d__5 = h__[k + 2 + (k + 2) * 
				h_dim1], std::abs(d__5)))) {
...
/*              .    unnecessary. ==== */
		...
		if (h__[k + 1 + k * h_dim1] != 0.) {
		    tst1 = (d__1 = h__[k + k * h_dim1], std::abs(d__1)) + (d__2 = 
			    h__[k + 1 + (k + 1) * h_dim1], std::abs(d__2));
		    if (tst1 == 0.) {
			if (k >= *ktop + 1) {
			    tst1 += (d__1 = h__[k + (k - 1) * h_dim1], std::abs(
				    d__1));
			}
			if (k >= *ktop + 2) {
			    tst1 += (d__1 = h__[k + (k - 2) * h_dim1], std::abs(
				    d__1));
			}
			if (k >= *ktop + 3) {
			    tst1 += (d__1 = h__[k + (k - 3) * h_dim1], std::abs(
				    d__1));
			}
			if (k <= *kbot - 2) {
...
/* Computing MAX */
		    ...
		    if ((d__1 = h__[k + 1 + k * h_dim1], std::abs(d__1)) <= std::max(
			    d__2,d__3)) {
/* Computing MAX */
			d__3 = (d__1 = h__[k + 1 + k * h_dim1], std::abs(d__1)), 
				d__4 = (d__2 = h__[k + (k + 1) * h_dim1], std::abs(
				d__2));
...
/* Computing MAX */
			d__3 = (d__1 = h__[k + 1 + (k + 1) * h_dim1], std::abs(
