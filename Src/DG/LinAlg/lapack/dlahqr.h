#ifndef BL_LAPACK_DLAHQR_H_
#define BL_LAPACK_DLAHQR_H_
...
/*        Look for a single small subdiagonal element. */
	...
	i__1 = l + 1;
	for (k = i__; k >= i__1; --k) {
	    if ((d__1 = h__[k + (k - 1) * h_dim1], std::abs(d__1)) <= smlnum) {
		goto L40;
	    }
	    tst = (d__1 = h__[k - 1 + (k - 1) * h_dim1], std::abs(d__1)) + (d__2 = 
		    h__[k + k * h_dim1], std::abs(d__2));
	    if (tst == 0.) {
		if (k - 2 >= *ilo) {
		    tst += (d__1 = h__[k - 1 + (k - 2) * h_dim1], std::abs(d__1));
		}
		if (k + 1 <= *ihi) {
		    tst += (d__1 = h__[k + 1 + k * h_dim1], std::abs(d__1));
		}
	    }
...
/*           .    improves accuracy in some cases.  ==== */
	    if ((d__1 = h__[k + (k - 1) * h_dim1], std::abs(d__1)) <= ulp * tst) {
/* Computing MAX */
		d__3 = (d__1 = h__[k + (k - 1) * h_dim1], std::abs(d__1)), d__4 = (
			d__2 = h__[k - 1 + k * h_dim1], std::abs(d__2));
...
/* Computing MAX */
		d__3 = (d__1 = h__[k + k * h_dim1], std::abs(d__1)), d__4 = (d__2 =
...
/*           Exceptional shift. */
	    ...
	    s = (d__1 = h__[l + 1 + l * h_dim1], std::abs(d__1)) + (d__2 = h__[l + 
		    2 + (l + 1) * h_dim1], std::abs(d__2));
...
/*           Exceptional shift. */
	    ...
	    s = (d__1 = h__[i__ + (i__ - 1) * h_dim1], std::abs(d__1)) + (d__2 = 
		    h__[i__ - 1 + (i__ - 2) * h_dim1], std::abs(d__2));
...
/*              ==== real shifts (use only one of them)  ==== */
		...
		rt1r = tr + rtdisc;
		rt2r = tr - rtdisc;
		if ((d__1 = rt1r - h22, std::abs(d__1)) <= (d__2 = rt2r - h22, std::abs(
			d__2))) {
		    rt1r *= s;
		    rt2r = rt1r;
		} else {
...
/*           overflows and most underflows.) */
	    ...
	    s = (d__1 = h__[m + m * h_dim1] - rt2r, std::abs(d__1)) + std::abs(rt2i) + 
		    std::abs(h21s);
	    ...
	    if ((d__1 = h__[m + (m - 1) * h_dim1], std::abs(d__1)) * (std::abs(v[1]) + 
		    std::abs(v[2])) <= ulp * std::abs(v[0]) * ((d__2 = h__[m - 1 + (m - 
		    1) * h_dim1], std::abs(d__2)) + (d__3 = h__[m + m * h_dim1], 
		    std::abs(d__3)) + (d__4 = h__[m + 1 + (m + 1) * h_dim1], std::abs(
		    d__4)))) {
		goto L60;
	    }
/* L50: */
