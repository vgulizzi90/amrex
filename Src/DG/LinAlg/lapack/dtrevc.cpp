...
/*     part of T to control overflow in triangular solver. */
	...
	for (i__ = 1; i__ <= i__2; ++i__) {
	    work[j] += (d__1 = t[i__ + j * t_dim1], std::abs(d__1));
/* L20: */
	}
/* L30: */
    }
...
/*           Compute the KI-th eigenvalue (WR,WI). */
	    ...
	    wi = 0.;
	    if (ip != 0) {
		wi = std::sqrt((d__1 = t[ki + (ki - 1) * t_dim1], std::abs(d__1))) * 
			std::sqrt((d__2 = t[ki - 1 + ki * t_dim1], std::abs(d__2)));
	    }
/* Computing MAX */
	    d__1 = ulp * (std::abs(wr) + std::abs(wi));
...
/*              Copy the vector x or Q*x to VR and normalize. */
		...
		if (! over) {
		    ...
		    remax = 1. / (d__1 = vr[ii + is * vr_dim1], std::abs(d__1));
...
/* L70: */
		    ...
		    remax = 1. / (d__1 = vr[ii + ki * vr_dim1], std::abs(d__1));
...
/*                [ (T(KI,KI-1)   T(KI,KI)   )               ] */
		...
		if ((d__1 = t[ki - 1 + ki * t_dim1], std::abs(d__1)) >= (d__2 = t[
			ki + (ki - 1) * t_dim1], std::abs(d__2))) {
		    work[ki - 1 + *n] = 1.;
...
/* Computing MAX */
			d__3 = emax, d__4 = (d__1 = vr[k + (is - 1) * vr_dim1]
				, std::abs(d__1)) + (d__2 = vr[k + is * vr_dim1], 
				std::abs(d__2));
...
/* Computing MAX */
			d__3 = emax, d__4 = (d__1 = vr[k + (ki - 1) * vr_dim1]
				, std::abs(d__1)) + (d__2 = vr[k + ki * vr_dim1], 
				std::abs(d__2));
			emax = std::max(d__3,d__4);
/* L120: */
		    }
...
/*                    Scale if necessary */
			...
			work[j + *n] = x[0];
/* Computing MAX */
			d__2 = (d__1 = work[j + *n], std::abs(d__1));
...
/* Computing MAX */
			d__3 = (d__1 = work[j + *n], std::abs(d__1)), d__4 = (d__2 
				= work[j + 1 + *n], std::abs(d__2)), d__3 = std::max(
				d__3,d__4);
...
/*              Copy the vector x or Q*x to VL and normalize. */
		...
		if (! over) {
		    ...
		    ii = idamax_(&i__2, &vl[ki + is * vl_dim1], &c__1) + ki - 
			    1;
		    remax = 1. / (d__1 = vl[ii + is * vl_dim1], std::abs(d__1));
		    i__2 = *n - ki + 1;
...
/* L180: */
		    ...
		    remax = 1. / (d__1 = vl[ii + ki * vl_dim1], std::abs(d__1));
...
/*                 ((T(KI+1,KI) T(KI+1,KI+1))                ) */
		...
		if ((d__1 = t[ki + (ki + 1) * t_dim1], std::abs(d__1)) >= (d__2 = 
			t[ki + 1 + ki * t_dim1], std::abs(d__2))) {
...
/* Computing MAX */
			d__3 = (d__1 = work[j + *n], std::abs(d__1)), d__4 = (d__2 
				= work[j + n2], std::abs(d__2)), d__3 = std::max(d__3,
				d__4);
			vmax = std::max(d__3,vmax);
			vcrit = bignum / vmax;
...
/*                    Scale if necessary */
			...
			work[j + 1 + n2] = x[3];
/* Computing MAX */
			d__1 = std::abs(x[0]), d__2 = std::abs(x[2]), d__1 = std::max(d__1,
				d__2), d__2 = std::abs(x[1]), d__1 = std::max(d__1,d__2)
				, d__2 = std::abs(x[3]), d__1 = std::max(d__1,d__2);
...
/*              Copy the vector x or Q*x to VL and normalize. */
		...
		if (! over) {
		    ...
		    for (k = ki; k <= i__2; ++k) {
/* Computing MAX */
			d__3 = emax, d__4 = (d__1 = vl[k + is * vl_dim1], std::abs(
...
/* L230: */
		    ...
		    for (k = 1; k <= i__2; ++k) {
/* Computing MAX */
			d__3 = emax, d__4 = (d__1 = vl[k + ki * vl_dim1], std::abs(
