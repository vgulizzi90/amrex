#ifndef BL_LAPACK_DLANGE_H_
#define BL_LAPACK_DLANGE_H_
...
/* Computing MAX */
		d__2 = value, d__3 = (d__1 = a[i__ + j * a_dim1], std::abs(d__1));
...
/*        Find norm1(A). */
	...
	for (j = 1; j <= i__1; ++j) {
	    ...
	    i__2 = *m;
	    for (i__ = 1; i__ <= i__2; ++i__) {
		sum += (d__1 = a[i__ + j * a_dim1], std::abs(d__1));
/* L30: */
	    }
...
/* L50: */
	...
	for (j = 1; j <= i__1; ++j) {
	    i__2 = *m;
	    for (i__ = 1; i__ <= i__2; ++i__) {
		work[i__] += (d__1 = a[i__ + j * a_dim1], std::abs(d__1));
/* L60: */
	    }
/* L70: */
	}
