#ifndef BL_LAPACK_DLANV2_H_
#define BL_LAPACK_DLANV2_H_
...
/*        Swap rows and columns */
	...
	*sn = 0.;
	...
	p = temp * .5;
/* Computing MAX */
	d__1 = std::abs(*b), d__2 = std::abs(*c__);
...
/*           Real eigenvalues. Compute A and D. */
	    ...
	    d__1 = std::sqrt(scale) * std::sqrt(z__);
	    z__ = p + d_sign(&d__1, &p);
	    *a = *d__ + z__;
	    *d__ -= bcmax / z__ * bcmis;
...
/*           Make diagonal elements equal. */
	    ...
	    *cs = std::sqrt((std::abs(sigma) / tau + 1.) * .5);
...
/*                    Real eigenvalues: reduce to upper triangular form */
			...
			sac = std::sqrt((std::abs(*c__)));
			d__1 = sab * sac;
			p = d_sign(&d__1, c__);
			tau = 1. / std::sqrt((d__1 = *b + *c__, std::abs(d__1)));
			*a = temp + p;
			*d__ = temp - p;
			*b -= *c__;
			*c__ = 0.;
...
/*     Store eigenvalues in (RT1R,RT1I) and (RT2R,RT2I). */
	...
	*rt1i = std::sqrt((std::abs(*b))) * std::sqrt((std::abs(*c__)));
	*rt2i = -(*rt1i);
    }
    return 0;
