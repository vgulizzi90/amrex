#ifndef BL_LAPACK_DLAQR1_H_
#define BL_LAPACK_DLAQR1_H_
...
/*     .. Executable Statements .. */
    ...
    /* Function Body */
    if (*n == 2) {
	s = (d__1 = h__[h_dim1 + 1] - *sr2, std::abs(d__1)) + std::abs(*si2) + (d__2 = 
		h__[h_dim1 + 2], std::abs(d__2));
	if (s == 0.) {
	    v[1] = 0.;
	    v[2] = 0.;
	} else {
	...
	s = (d__1 = h__[h_dim1 + 1] - *sr2, std::abs(d__1)) + std::abs(*si2) + (d__2 = 
		h__[h_dim1 + 2], std::abs(d__2)) + (d__3 = h__[h_dim1 + 3], std::abs(
		d__3));
	if (s == 0.) {
	    v[1] = 0.;
	    v[2] = 0.;
