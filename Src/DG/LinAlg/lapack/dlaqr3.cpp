...
/*        ==== 1-by-1 deflation window: not much to do ==== */
	...
	*nd = 0;
/* Computing MAX */
	d__2 = smlnum, d__3 = ulp * (d__1 = h__[kwtop + kwtop * h_dim1], std::abs(
		d__1));
	if (std::abs(s) <= std::max(d__2,d__3)) {
	    *ns = 0;
	    *nd = 1;
	    if (kwtop > *ktop) {
...
/*     .    here and there to keep track.) ==== */
    ...
    nmin = ilaenv_(&c__12, "DLAQR3", &jw, &c__1, &jw, lwork);
    if (jw > nmin) {
...
/*           ==== Real eigenvalue ==== */
	    ...
	    foo = (d__1 = t[*ns + *ns * t_dim1], std::abs(d__1));
	    if (foo == 0.) {
		foo = std::abs(s);
	    }
/* Computing MAX */
	    d__2 = smlnum, d__3 = ulp * foo;
	    if ((d__1 = s * v[*ns * v_dim1 + 1], std::abs(d__1)) <= std::max(d__2,d__3))
		     {
...
/*           ==== Complex conjugate pair ==== */
	    ...
	    foo = (d__3 = t[*ns + *ns * t_dim1], std::abs(d__3)) + std::sqrt((d__1 = t[*
		    ns + (*ns - 1) * t_dim1], std::abs(d__1))) * std::sqrt((d__2 = t[*
		    ns - 1 + *ns * t_dim1], std::abs(d__2)));
	    if (foo == 0.) {
		foo = std::abs(s);
	    }
/* Computing MAX */
	    d__3 = (d__1 = s * v[*ns * v_dim1 + 1], std::abs(d__1)), d__4 = (d__2 =
		     s * v[(*ns - 1) * v_dim1 + 1], std::abs(d__2));
/* Computing MAX */
...
L40:
	if (k <= kend) {
	    if (k == i__ + 1) {
		evi = (d__1 = t[i__ + i__ * t_dim1], std::abs(d__1));
	    } else {
		evi = (d__3 = t[i__ + i__ * t_dim1], std::abs(d__3)) + std::sqrt((d__1 =
			 t[i__ + 1 + i__ * t_dim1], std::abs(d__1))) * std::sqrt((d__2 =
			 t[i__ + (i__ + 1) * t_dim1], std::abs(d__2)));
	    }

	    if (k == kend) {
		evk = (d__1 = t[k + k * t_dim1], std::abs(d__1));
	    } else if (t[k + 1 + k * t_dim1] == 0.) {
		evk = (d__1 = t[k + k * t_dim1], std::abs(d__1));
	    } else {
		evk = (d__3 = t[k + k * t_dim1], std::abs(d__3)) + std::sqrt((d__1 = t[
			k + 1 + k * t_dim1], std::abs(d__1))) * std::sqrt((d__2 = t[k + 
			(k + 1) * t_dim1], std::abs(d__2)));
	    }
