...
/*        .    NWR.GE.3.) ==== */
	...
	nwr = ilaenv_(&c__13, "DLAQR0", n, ilo, ihi, lwork);
	nwr = std::max(2,nwr);
/* Computing MIN */
...
/*        .    and greater than or equal to two as required. ==== */
	...
	nsr = ilaenv_(&c__15, "DLAQR0", n, ilo, ihi, lwork);
/* Computing MIN */
...
/*        ==== DLAHQR/DLAQR0 crossover point ==== */
	...
	nmin = ilaenv_(&c__12, "DLAQR0", n, ilo, ihi, lwork);
	nmin = std::max(11,nmin);

/*        ==== Nibble crossover point ==== */

	nibble = ilaenv_(&c__14, "DLAQR0", n, ilo, ihi, lwork);
	nibble = std::max(0,nibble);

/*        ==== Accumulate reflections during ttswp?  Use block */
/*        .    2-by-2 structure during matrix-matrix multiply? ==== */

	kacc22 = ilaenv_(&c__16, "DLAQR0", n, ilo, ihi, lwork);
	kacc22 = std::max(0,kacc22);
...
/* Computing MIN */
		...
		if (nw >= nh - 1) {
		    ...
		    kwtop = kbot - nw + 1;
		    if ((d__1 = h__[kwtop + (kwtop - 1) * h_dim1], std::abs(d__1)) 
...
/*           .    or if the remaining active block is small. */
	    ...
	    if (ld == 0 || (ld * 100 <= nw * nibble && kbot - ktop + 1 > std::min(nmin,nwmax))) {
...
/* Computing MAX */
			...
			ss = (d__1 = h__[i__ + (i__ - 1) * h_dim1], std::abs(d__1))
...
/*                    .    pairs together. ==== */
			...
			for (k = kbot; k >= i__2; --k) {
				...
				if ((d__1 = wr[i__], std::abs(d__1)) + (d__2 = wi[
					i__], std::abs(d__2)) < (d__3 = wr[i__ + 1]
					, std::abs(d__3)) + (d__4 = wi[i__ + 1], 
					std::abs(d__4))) {
				    sorted = FALSE_;
...
/*              .    real, then use only one.  ==== */
		...
		if (kbot - ks + 1 == 2) {
		    if (wi[kbot] == 0.) {
			if ((d__1 = wr[kbot] - h__[kbot + kbot * h_dim1], std::abs(
				d__1)) < (d__2 = wr[kbot - 1] - h__[kbot + 
				kbot * h_dim1], std::abs(d__2))) {
			    wr[kbot - 1] = wr[kbot];
			} else {
			    wr[kbot] = wr[kbot - 1];
			}
