#ifndef BL_LAPACK_DLANSY_H_
#define BL_LAPACK_DLANSY_H_
...
/* Computing MAX */
		    d__2 = value, d__3 = (d__1 = a[i__ + j * a_dim1], std::abs(
			    d__1));
...
/*        Find normI(A) ( = norm1(A), since A is symmetric). */
	...
	if (lsame_(uplo, "U")) {
		...
		i__2 = j - 1;
		for (i__ = 1; i__ <= i__2; ++i__) {
		    absa = (d__1 = a[i__ + j * a_dim1], std::abs(d__1));
		    sum += absa;
		    work[i__] += absa;
/* L50: */
		}
		work[j] = sum + (d__1 = a[j + j * a_dim1], std::abs(d__1));
/* L60: */
	    }
	    i__1 = *n;
...
/* L80: */
	    ...
	    i__1 = *n;
	    for (j = 1; j <= i__1; ++j) {
		sum = work[j] + (d__1 = a[j + j * a_dim1], std::abs(d__1));
		i__2 = *n;
