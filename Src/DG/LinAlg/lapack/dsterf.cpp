...
L10:
    ...
    i__1 = *n - 1;
    for (m = l1; m <= i__1; ++m) {
	if ((d__3 = e[m], std::abs(d__3)) <= std::sqrt((d__1 = d__[m], std::abs(d__1))) * 
		std::sqrt((d__2 = d__[m + 1], std::abs(d__2))) * eps) {
	    e[m] = 0.;
	    goto L30;
	}
/* L20: */
...
L50:
	if (l != lend) {
	    i__1 = lend - 1;
	    for (m = l; m <= i__1; ++m) {
		if ((d__2 = e[m], std::abs(d__2)) <= eps2 * (d__1 = d__[m] * d__[m 
			+ 1], std::abs(d__1))) {
		    goto L70;
		}
/* L60: */
...
L100:
	i__1 = lend + 1;
	for (m = l; m >= i__1; --m) {
	    if ((d__2 = e[m - 1], std::abs(d__2)) <= eps2 * (d__1 = d__[m] * d__[m 
		    - 1], std::abs(d__1))) {
		goto L120;
	    }
/* L110: */
