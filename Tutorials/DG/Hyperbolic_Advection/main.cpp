...
//
// rho_{,t}+(a_i rho)_{,i} = 0.
//
// where rho is the advected field and a = {a_i} is a divergence-free
// velocity field.
// The problem is subjected to:
// i) either periodic or inflow boundary conditions
// ii) initial conditions: rho(0,x) = rho0(x)
//
// ####################################################################
// SELECT SET OF PDES =================================================
...
amrex::Print() << "#######################################################################" << std::endl;
    ...
    */
    ...
    if (inputs.plot_int > 0)
    {
        ...
        std::vector<std::string> field_names = {"rho", "err"};
