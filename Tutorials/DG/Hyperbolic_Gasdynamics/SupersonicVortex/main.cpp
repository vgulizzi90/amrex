...
amrex::Print() << "# supersonic vortex problem.                                           " << std::endl;
amrex::Print() << "#                                                                      " << std::endl;
amrex::Print() << "#######################################################################" << std::endl;
...
amrex::Print() << "#######################################################################" << std::endl;
    ...
    // DO THE ANALYSIS
    {
        ...
        {
            amrex::Real dom_err;
            dom_err = amrex::DG::EvalError(0.0, mesh, matfactory, DG_N_SOL, X, IG);
        ...
        amrex::Print() << "# START OF THE ANALYSIS" << std::endl;
        {
            ...
            while ((t < inputs.time.T*(1.0-1.0e-12)) && (n < inputs.time.n_steps))
            {
                ...
                // EVAL ERROR
                {
                    dom_err = amrex::DG::EvalError(t, mesh, matfactory, DG_N_SOL, X, IG);
                }
        ...
        {
            const amrex::Real dom_err = amrex::DG::EvalError(inputs.time.T, mesh, matfactory, DG_N_SOL, X, IG);
