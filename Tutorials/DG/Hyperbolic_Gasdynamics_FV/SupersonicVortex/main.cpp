...
// supersonic vortex problem using a Finite Volume scheme and the
...
amrex::Print() << "# supersonic vortex problem using a Finite Volume scheme and the       " << std::endl;
...
amrex::Print() << "#######################################################################" << std::endl;
    ...
    const std::string problem = "PROBLEM_SupersonicVortex";
    ...
    // DO THE ANALYSIS
    {
        ...
        // INIT MULTIFAB ==============================================
        {
            ...
            Xr.define(mesh.cc_ba, mesh.dm, X_n_comp, ngr);
        }
        // ============================================================
        ...
        amrex::MultiFab::Copy(Xr, X, 0, 0, DG_N_SOL, ngr);

        // EVAL ERROR
        {
            amrex::Real dom_err, bou_err;
            amrex::FV::EvalError(0.0, mesh, matfactory, X, IG, dom_err, bou_err);
        ...
        {
            int n = 0;
            amrex::Real t, dt, dom_err, bou_err, max_diff_rho;
            ...
            dt = 0.0;
            max_diff_rho = 1.0;
            while ((t < inputs.time.T*(1.0-1.0e-12)) && (n < inputs.time.n_steps) && (max_diff_rho > 1.0e-10))
            {
                ...
                // EVAL ERROR
                {
                    amrex::FV::EvalError(t, mesh, matfactory, X, IG, dom_err, bou_err);
                }

                // EVAL MAXIMUM DIFFERENCE IN DENSITY
                max_diff_rho = amrex::FV::EvalMaxDiff(mesh, matfactory, X, Xr, IG);
                amrex::MultiFab::Copy(Xr, X, 0, 0, DG_N_SOL, ngr);

                // WRITE TO OUTPUT
                if ((inputs.plot_int > 0) && ((n%inputs.plot_int == 0) || (std::abs(t/inputs.time.T-1.0) < 1.0e-12) || (max_diff_rho < 1.0e-10)))
                {
                    amrex::FV::Export_VTK(output_folderpath, n, inputs.time.n_steps, "Solution",
                                          t, mesh, matfactory, X,
                                          IG);
                ...
                amrex::Print() << std::scientific << std::setprecision(5) << std::setw(12)
                               << dt << ", t = " << t << ", dom_err = " << dom_err << ", diff_rho = " << max_diff_rho
                               << ", tts [s] = " << tps 
        ...
        {
            amrex::Real dom_err, bou_err;
            amrex::FV::EvalError(inputs.time.T, mesh, matfactory, X, IG, dom_err, bou_err);
